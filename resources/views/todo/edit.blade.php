...
<form method="POST" action="{{route('todo.store')}}" class="postEditMain--form" enctype="multipart/form-data">
    ...
    <ul>
      <li>
        <div>タイトル</div>
      <input type="text" name="title" class="postEditMain--title" value="{{$todo->title}}">
      </li>

      <li>
        <div>コメント</div>
      <textarea rows="10" cols="40"  name="body" class="postEditMain--textArea" placeholder="メッセージを入力してください">{{$todo->body}}</textarea>
      </li>
      <li>
